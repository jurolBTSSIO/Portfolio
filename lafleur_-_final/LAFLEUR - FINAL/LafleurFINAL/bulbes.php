...
<table border=1>
   	<tr>
        ...
        $connexion = new PDO('mysql:host=localhost;dbname=lafleur'
                ,$login , $mdp);
        $resultat = $connexion->query('SELECT * FROM produit WHERE pdt_categorie ="bul";');
            
        foreach($resultat as $ligne)
        {
            echo '<center><tr><td>'.$ligne['pdt_ref'].'</td><td>'.$ligne['pdt_designation'].
                    "</td><td> ".$ligne['pdt_image'].'</td><td>'.$ligne['pdt_prix'].
                    '&#128;</td></tr></center>';
            
        }
            echo '</table><br>';
        $result = $connexion->query('SELECT pdt_ref,pdt_designation FROM produit WHERE pdt_ref LIKE "b%" ;');
        ...
        echo '<input type="number" name="quantite" min="1" max="9999"  /><input type="submit" value="Ajoutez au panier" name="valider1"></form>';
