...
<table border=1>
   	<tr>
        ...
        $connexion = new PDO('mysql:host=localhost;dbname=lafleur'
                ,$login , $mdp);
        $resultat = $connexion->query('SELECT * FROM produit WHERE pdt_categorie ="mas";');
        ...
        foreach($resultat as $ligne)
        {
            ...
            echo '</table><br>';
             $result = $connexion->query('SELECT pdt_ref,pdt_designation FROM produit WHERE pdt_ref LIKE "m%" ;');
        ...
        echo '<input type="number" name="quantite" min="1" max="9999"  /><input type="submit" value="Ajoutez au panier" name="valider2"></form>';
